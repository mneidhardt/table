    ...
    public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals(Constants.NEWROW)) {
			...
			try {
				...
				if (reply == JFileChooser.APPROVE_OPTION) {
					...
					if (fc.getSelectedFile().isFile()) {
						if (JOptionPane.showConfirmDialog(null, Constants.FILEEXISTSOVERWRITE, "what here???",
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Failure when opening file " + filename + ": " + e1.getMessage());
			}
